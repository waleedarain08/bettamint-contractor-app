...
const projectSlice = createSlice({
  ...
  reducers: {
    ...
    emptyProjectsSimple: (state) => {
      state.projectsListSimple = null;
    },
...
export const emptyAllProjectsSimpleAction = () => async (dispatch) => {
  dispatch(emptyProjectsSimple());
};
...
export const getProjectsForLabour = (token, projectId) => async (dispatch) => {
  try {
    ...
    await api
      ...
      .request(
        "POST",
        PROJECT_USER_LIST_URL + `?projectId=${projectId}`,
        null,
        {
          Authorization: token,
        }
